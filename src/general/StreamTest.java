...
public class StreamTest {
    ...
    @Test
    public void test1() {
        List<InterviewNotesTest.Interval> intervals1 = new LinkedList<>();
        intervals1.add(new InterviewNotesTest.Interval(1, 2));
        intervals1.add(new InterviewNotesTest.Interval(0, 2));
        Set<Integer> starts = intervals1.stream().sorted().map(a -> a.start).collect(Collectors.toSet());
...
//Find the average salary by department, sorted by average salary in descending order

    public void testEmployee() {
        List<Employee> employees = new ArrayList<>();

        employees.stream().collect(
                        Collectors.groupingBy(e -> e.department, Collectors.averagingDouble(e -> e.salary)))
                .entrySet().stream().sorted((Comparator.comparingDouble(Map.Entry<String, Double>::getValue)).reversed());
