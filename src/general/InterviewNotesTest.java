...
// compare by value
    ...
    public void testMapEq() {
        ...
        charToInt.put('a', 1);
        charToInt.put('b', 2);
        Map<Character, Integer> charToInt1 = new HashMap<>();
        charToInt1.put('a', 1);
        charToInt1.put('b', 2);
        System.out.println(charToInt.equals(charToInt1));

        Map<Character, Integer> charToInt2 = new HashMap<>();
        List<String> strs = List.of("abcd", "bcda");

        for (String str : strs) {
            Map<Map<Character, Integer>, List<String>> chardToListStr = new HashMap<>();
            Map<Character, Integer> key = new HashMap<>();
            for (Character ch : str.toCharArray()) {
                key.computeIfPresent(ch, (k, v) -> v + 1);
                key.computeIfAbsent(ch, (k) -> 1);
            }
        }
